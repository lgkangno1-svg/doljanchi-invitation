...
export const appRouter = router({
  ...
  invitation: router({
    ...
    addRsvp: publicProcedure.input(z.object({ name: z.string().trim().min(1).max(80), companionNames: z.array(z.string().trim().min(1).max(80)).max(19).default([]), attendance: z.enum(["attending", "unable"]), adults: z.number().int().min(0).max(10), children: z.number().int().min(0).max(10), meal: z.boolean().optional(), contact: z.string().trim().max(40).optional(), note: z.string().trim().max(300).optional() })).mutation(async ({ input }) => { const invite = await db.getOrCreateInvitation(); return db.createRsvp({ ...input, invitationId: invite.id, companionNames: JSON.stringify(input.companionNames), meal: input.meal === false ? 0 : 1, contact: input.contact || null, note: input.note || null }); }),
  }),
  admin: router({
