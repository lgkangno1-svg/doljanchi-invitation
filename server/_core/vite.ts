...
const head = () => { const title = "서아의 첫 번째 생일에 초대합니다"; const description = "소중한 분들과 함께하는 서아의 첫 번째 생일에 초대합니다."; const image = process.env.CANONICAL_ORIGIN ? `${process.env.CANONICAL_ORIGIN}/manus-storage/og-invitation_44baa7a2.svg` : "/manus-storage/og-invitation_44baa7a2.svg"; return `<title>${esc(title)}</title><meta name="description" content="${esc(description)}" /><meta name="robots" content="noindex, nofollow" /><meta property="og:type" content="website" /><meta property="og:title" content="${esc(title)}" /><meta property="og:description" content="${esc(description)}" /><meta property="og:image" content="${esc(image)}" /><meta property="og:image:alt" content="서아의 첫 번째 생일 초대장" />${process.env.CANONICAL_ORIGIN ? `<meta property="og:url" content="${esc(process.env.CANONICAL_ORIGIN)}" /><link rel="canonical" href="${esc(process.env.CANONICAL_ORIGIN)}" />` : ""}`; };
