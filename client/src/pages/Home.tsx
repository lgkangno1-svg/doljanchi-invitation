...
export default function Home() {
  ...
  const addGuestbook = trpc.invitation.addGuestbook.useMutation({ onSuccess: () => { guestbook.refetch(); setGuestName(""); setGuestCompanions([]); setGuestMessage(""); toast.success("채원이에게 축하 메시지가 전달되었어요."); } });
  ...
  const [rsvp, setRsvp] = useState({ name: "", companionNames: [] as string[], attendance: "attending" as "attending" | "unable", adults: 1, children: 0, contact: "", note: "" });
  ...
  const submitRsvp = (event: React.FormEvent) => { event.preventDefault(); if (!rsvp.name.trim()) return toast.error("성함을 입력해 주세요."); addRsvp.mutate({ ...rsvp, companionNames: normalizeCompanionNames(rsvp.companionNames) }); };
  ...
  return <main className="hotel-invitation">
    ...
    <Section label="RESERVATION" className="rsvp-hotel"><div className="rsvp-heading"><h2>참석 여부를<br /><em>알려주세요</em></h2><p>행사 준비를 위해 소중한 응답을 부탁드립니다.</p></div>{rsvpSent ? <div className="rsvp-complete"><span>THANK YOU</span><b>응답이 전달되었습니다</b><p>채원이의 첫 생일에 함께해 주셔서 감사합니다.</p></div> : <form onSubmit={submitRsvp}><div className="attendance-options"><button type="button" onClick={() => setRsvp({ ...rsvp, attendance: "attending" })} className={rsvp.attendance === "attending" ? "active" : ""}>참석하겠습니다</button><button type="button" onClick={() => setRsvp({ ...rsvp, attendance: "unable" })} className={rsvp.attendance === "unable" ? "active" : ""}>참석이 어렵습니다</button></div><label>대표 성함<input value={rsvp.name} onChange={event => setRsvp({ ...rsvp, name: event.target.value })} maxLength={80} /></label><CompanionFields title="함께 참석하는 일행" labelPrefix="일행 성함" max={19} names={rsvp.companionNames} guidance={<><strong>참석하시는 모든 분의 성함을 정확히 적어 주세요.</strong><span>대표 성함 외 함께 오시는 분은 오른쪽 <b>+ 일행 추가</b> 버튼을 눌러 한 분씩 입력해 주세요.</span></>} onChange={companionNames => setRsvp({ ...rsvp, companionNames })} onAdd={() => setRsvp({ ...rsvp, companionNames: addCompanionInput(rsvp.companionNames, 19) })} onRemove={index => setRsvp({ ...rsvp, companionNames: removeCompanionInput(rsvp.companionNames, index) })} />{rsvp.attendance === "attending" && <div className="guest-counts"><label>성인<input type="number" value={rsvp.adults} min={0} max={10} onChange={event => setRsvp({ ...rsvp, adults: Number(event.target.value) })} /></label><label>아이<input type="number" value={rsvp.children} min={0} max={10} onChange={event => setRsvp({ ...rsvp, children: Number(event.target.value) })} /></label></div>}<label>연락처 <small>(선택)</small><input value={rsvp.contact} onChange={event => setRsvp({ ...rsvp, contact: event.target.value })} maxLength={40} /></label><label>전하고 싶은 말씀 <small>(선택)</small><textarea value={rsvp.note} onChange={event => setRsvp({ ...rsvp, note: event.target.value })} maxLength={300} /></label><button className="hotel-primary" disabled={addRsvp.isPending}>응답 보내기</button><p className="form-privacy">연락처는 행사 안내를 위해서만 사용하며, 행사 종료 후 정리합니다.</p></form>}</Section>
