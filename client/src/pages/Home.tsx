...
export default function Home() {
  ...
  return <main className="hotel-invitation">
    ...
    <Section label="A YEAR OF JOY" className="editorial-gallery"><h2>한 해 동안 피어난<br /><em>우리의 기쁨</em></h2><div className="gallery-editorial"><InvitationMediaView className="gallery-portrait" media={galleryMedia[0] ?? null} fallback={HERO_IMAGE} alt="흰 레이스 의상을 입은 채원이" /><div className="gallery-side-story"><p>채원이와 함께한<br />가장 따스한 계절</p><InvitationMediaView className="gallery-detail" media={galleryMedia[1] ?? null} fallback={HERO_IMAGE} alt="작고 사랑스러운 채원이의 발" /></div></div></Section>

    <section className="love-transition"><InvitationMediaView className="seasonal-transition-media" media={galleryMedia[2] ?? null} fallback={HERO_IMAGE} alt="벚꽃 아래 환하게 웃는 채원이" /><div><p>A year of love,</p><strong>a lifetime of joy.</strong></div></section>
    {galleryMedia.slice(3).length > 0 && <section className="memory-strip"><div className="extra-gallery">{galleryMedia.slice(3).map((media, index) => <InvitationMediaView className={index === 0 ? "gallery-hands" : "gallery-blossom"} key={`${media.url}-${index}`} media={media} fallback={HERO_IMAGE} alt={index === 0 ? "가족의 손을 잡은 채원이" : "벚꽃 아래의 채원이"} />)}</div></section>}
